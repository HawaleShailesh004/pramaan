/**
 * Deploy silent-bell contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
import { emptyPramaanState, pramaanWitnesses } from "./pramaan-witnesses";
import { PRAMAAN_SECRETS } from "./pramaan-keys";
import { USAR_ISSUER_ID } from "./pramaan-leaf";
import { savePramaanDeployment } from "./pramaan-state";
...
const compiledContractFactory: any = CompiledContract.make(
  "pramaan",
  Pramaan.Contract,
);
...
async function main() {
  ...
  console.log(`║  Deploy pramaan to ${network}`);
  console.log(
    "╚══════════════════════════════════════════════════════════════╝\n",
  );
  ...
  savePramaanDeployment(network, contractAddress);
  console.log("  Saved to .pramaan-state.json\n");
  ...
  console.log("  Next: npm run demo:flow && npm run api\n");
}
